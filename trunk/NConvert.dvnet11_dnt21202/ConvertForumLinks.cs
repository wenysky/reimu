...
namespace NConvert.dvnet11_dnt21202
{
    public partial class Provider : IProvider
    {
        ...
        {
            return Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT([Id]) FROM {0}Link", MainForm.cic.SrcDbTablePrefix)));
        }

        public List<ForumLinks> GetForumLinkList()
        {
            string sqlTopicTypes = string.Format("SELECT * FROM {0}Link ORDER BY Id", MainForm.cic.SrcDbTablePrefix);
