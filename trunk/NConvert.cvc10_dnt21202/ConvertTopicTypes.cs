...
namespace NConvert.cvc10_dnt21202
{
    public partial class Provider : IProvider
    {
        ...
        {
            return Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT([Id]) FROM {0}Subject", MainForm.srcDbTableProfix)));
        ...
        public List<TopicTypes> GetTopicTypeList()
        {
            string sqlTopicTypes = string.Format
                       ("SELECT * FROM {0}Subject ORDER BY Id", MainForm.srcDbTableProfix);            
