...
namespace NConvert.cvc20beta_dnt21202
{
    public partial class Provider : IProvider
    {
        ...
        public List<Topics> GetTopicList(int CurrentPage)
        {
            ...
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}Topic ORDER BY Id", MainForm.cic.SrcDbTablePrefix, MainForm.PageSize);
            }
            else
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}Topic WHERE Id NOT IN (SELECT TOP {2} Id FROM {0}Topic ORDER BY Id) ORDER BY Id", MainForm.cic.SrcDbTablePrefix, MainForm.PageSize, MainForm.PageSize * (CurrentPage - 1));
            }
            #endregion
