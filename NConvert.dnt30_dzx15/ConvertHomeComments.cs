...
namespace NConvert.dnt30_dzx15
{
    public partial class Provider : IProvider
    {
        ...
        public int GetHomeCommentRecordCount()
        {
            ...
            {
                return 2 + Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT(id) FROM [science].[dbo].[kexue_blogbook]")));
            }
            else
            {
        ...
        public List<CommentInfo> GetHomeCommentList(int pagei)
        {
            string sqlBoard = string.Format(
                "SELECT * FROM [science].[dbo].[kexue_blogbook]",
