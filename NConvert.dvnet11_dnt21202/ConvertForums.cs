...
namespace NConvert.dvnet11_dnt21202
{
    public partial class Provider : IProvider
    {
        public int GetForumsRecordCount()
        {
            ...
            {
                return Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT([Id]) FROM (SELECT [Id] FROM {0}Board UNION ALL SELECT [Id] FROM {0}Category)a", MainForm.cic.SrcDbTablePrefix)));
            }
            else
            {
        ...
        public List<Forums> GetForumList()
        {
            string sqlBoard = string.Format
                       ("SELECT Id,CategoryId,ParentId,SortId,BoardName,Description,Topics,Replies,Childs,LastTopicId,LastTopicTitle,LastTopicTime,LastTopicUID,LastTopicAuthor,Announcement,Hidden,ChildHorizontal,ChildColumns FROM {0}Board ORDER BY Id", MainForm.cic.SrcDbTablePrefix);

            int maxfid = Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT MAX(Id) FROM {0}Board", MainForm.cic.SrcDbTablePrefix)));//取得最大的自增号,重排CategoryId
            ...
            string sqlCategory = string.Format
                       ("SELECT Id,CategoryName,Description,Horizontal,Columns,Visible,SortId FROM {0}Category ORDER BY Id", MainForm.cic.SrcDbTablePrefix);
