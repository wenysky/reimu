...
namespace NConvert.cvc10_dnt21202
{
    public partial class Provider : IProvider
    {
        ...
        {
            return Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT([Id]) FROM {0}Attach", MainForm.srcDbTableProfix)));
        }
        ...
        public List<Attachments> GetAttachmentList(int CurrentPage)
        {
            ...
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}Attach ORDER BY Id", MainForm.srcDbTableProfix, MainForm.PageSize);
            }
            else
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}Attach WHERE Id NOT IN (SELECT TOP {2} Id FROM {0}Attach) ORDER BY Id", MainForm.srcDbTableProfix, MainForm.PageSize, MainForm.PageSize * (CurrentPage - 1));
            }
            #endregion
