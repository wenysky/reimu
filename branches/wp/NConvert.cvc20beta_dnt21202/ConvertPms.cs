...
namespace NConvert.cvc20beta_dnt21202
{
    public partial class Provider : IProvider
    {
        ...
        {
            return Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT([Id]) FROM {0}Message", MainForm.cic.SrcDbTablePrefix)));
        }
        ...
        public List<Pms> GetPmList(int CurrentPage)
        {
            ...
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}Message ORDER BY Id", MainForm.cic.SrcDbTablePrefix, MainForm.PageSize);
            }
            else
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}Message WHERE Id NOT IN (SELECT TOP {2} Id FROM {0}Message ORDER BY Id) ORDER BY Id", MainForm.cic.SrcDbTablePrefix, MainForm.PageSize, MainForm.PageSize * (CurrentPage - 1));
            }
            #endregion
