...
namespace NConvert.dnt30_dzx15
{
    public partial class Provider : IProvider
    {
        ...
        {
            return 3 * Convert.ToInt32(MainForm.srcDBH.ExecuteScalar(string.Format("SELECT COUNT(pmid) FROM {0}pms WHERE folder=0", MainForm.cic.SrcDbTablePrefix)));//Discuz!X1.5统一了发件箱和收件箱，所以只需要转发送者的那一条就够了；因为一条数据会被插入3次，所以*3
        }
        ...
        public List<Pms> GetPmList(int CurrentPage)
        {
            ...
            {
                sql = string.Format
                       ("SELECT TOP {1} * FROM {0}pms WHERE WHERE folder=0 AND pmid NOT IN (SELECT TOP {2} pmid FROM {0}pms WHERE folder=0 ORDER BY pmid) ORDER BY pmid", MainForm.cic.SrcDbTablePrefix, MainForm.PageSize, MainForm.PageSize * (CurrentPage - 1));
            }
            #endregion
