    ...
    public partial class Provider : IProvider
    {
        ...
        public List<Forums> GetForumList()
        {
            ...
            while (drBoard.Read())
            {
                ...
                objForum.viewperm = string.Format("\t{0}\t", drBoard["viewperm"].ToString().Replace(",", "\t"));
                objForum.postperm = string.Format("\t{0}\t", drBoard["postperm"].ToString().Replace(",", "\t"));
                objForum.replyperm = string.Format("\t{0}\t", drBoard["replyperm"].ToString().Replace(",", "\t"));
                objForum.getattachperm = string.Format("\t{0}\t", drBoard["getattachperm"].ToString().Replace(",", "\t"));
                objForum.postattachperm = string.Format("\t{0}\t", drBoard["postattachperm"].ToString().Replace(",", "\t"));
                objForum.postimageperm = string.Format("\t{0}\t", drBoard["postattachperm"].ToString().Replace(",", "\t"));//Discuz!NT无这个，统一为上传附件权限
                objForum.spviewperm = "";
                objForum.keywords = "";
