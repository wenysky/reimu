...
namespace NConvert.Entity
{
    ...
    public class UserGroupInfo
    {
        ...
        private int m_groupid = -99;	//用户组ID
        private int m_radminid = 0;	//关联管理组ID
        private int m_type = 0;	//默认是私有,公用可以自己加入
        private int m_system = 0;	//用户组类型
        private string m_grouptitle = "";	//用户组名称
        private int m_creditshigher = 0;	//积分下限
        private int m_creditslower = 0;	//积分上限
        private int m_stars = 0;	//星星数目
        private string m_color = "";	//名称颜色
        private string m_groupavatar = "";	//用户组头像
        private int m_readaccess = 0;	//阅读权限
        private int m_allowvisit = 1;	//是否允许访问论坛
        private int m_allowpost = 1;	//是否允许发新主题
        private int m_allowreply = 1;	//是否允许回复
        private int m_allowpostpoll = 1;	//是否允许发起投票
        private int m_allowdirectpost = 0;	//允许使用 html 代码
        private int m_allowgetattach = 1;	//是否允许下载附件
        private int m_allowpostattach = 1;	//是否允许发布附件
        private int m_allowvote = 1;	//是否允许参与投票
        private int m_allowmultigroups = 0;	//是否允许加入多个用户组
        private int m_allowsearch = 1;	//是否允许搜索
        //		private int m_allowavatar;	//是否允许使用头像, 0=不允许, 1=允许使用系统自带头像, 2=允许使用Url地址头像(且包括1), 3允许使用上传头像(且包括1和2)
        private int m_allowcstatus = 1;	//是否允许自定义头衔
        private int m_allowuseblog = 1;	//是否允许使用blog
        private int m_allowinvisible = 1;	//是否允许隐身
        private int m_allowtransfer = 1;	//是否允许积分转账
        private int m_allowsetreadperm = 1;	//是否允许设置阅读积分权限
        private int m_allowsetattachperm = 1;	//是否允许设置下载积分限制
        private int m_allowhidecode = 1;	//是否允许使用hide代码
        private int m_allowhtml = 0;	//是否允许发布html帖
        private int m_allowcusbbcode = 1;	//是否允许使用Discuz!NT代码
        private int m_allownickname = 1;	//是否允许使用昵称
        private int m_allowsigbbcode = 1;	//签名是否支持Discuz!NT代码
        private int m_allowsigimgcode = 1;	//签名是否支持图片代码
        private int m_allowviewpro = 1;	//是否允许查看用户资料
        private int m_allowviewstats = 1;	//是否允许查看统计
        private int m_disableperiodctrl = 0;	//是否不受时间段限制
        private int m_reasonpm = 3;	//是否将操作理由短消息通知作者
        private int m_maxprice = 1;	//主题最高售价
        private int m_maxpmnum = 100;	//短消息最多条数
        private int m_maxsigsize = 1000;	//签名最多字节
        private int m_maxattachsize = 1000;	//附件最大尺寸
        private int m_maxsizeperday = 10000;	//每天最大附件总尺寸
